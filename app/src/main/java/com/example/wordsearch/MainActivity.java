...
public class MainActivity extends AppCompatActivity {
    ...
    GridView gridView;
    Button checkButton,replayButton;
    ImageButton imageButtonNextLevel;
    TextView timerTextView, lostTextView;
    int vrijemeZaLevelUMinutama = 1;
    boolean provjeraDaLiJeTajmerProsao = false;
    //Podaci koji se snimaju u bazu
    int levelBaza = 1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lostTextView = (TextView)findViewById(R.id.lostTextView);
        imageButtonNextLevel = (ImageButton)findViewById(R.id.imageButtonNextLevel);
        replayButton = (Button)findViewById(R.id.replayButton);
//Ako je korisnik vec igrao prije i ima veci level, odma namjesta taj level.
        levelTextView.setText("Level " + (int) (currentLevelInGame));
        ...
        gridAdapter = new GridAdapter(MainActivity.this, arrayOfCurrentLetters);
    ...
    public void checkButton(View view) {
        ...
        if((sumOfTotalPoints) >= 3){
            while(onlyOnceAndLevelDone){
                openDialog();
                onlyOnceAndLevelDone = false;
            }
        }
    }
    boolean onlyOnceAndLevelDone = true; // za checkButton, da samo jednom ponudi da se ide na sljedeci level
    //OTVARANJE DIJALOGA I PREBACIVANJE NA NOVI LEVEL
    public void openDialog(){
        AlertDialog dialog = new AlertDialog.Builder(this)
                .setTitle("Next level")
                .setMessage("Do you want to go on next level?")
                .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        controlTimerReset = true;
                        if(anim != null)
                        anim.cancel(); // prekida animaciju slicice za nextlevel, jer ako se ne prekine ostaje prikazana i kad ne treba.
                        goToNextLevelMethod();
                    }
                })
                .setNegativeButton("Stay on this level", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        imageButtonNextLevel.setVisibility(View.VISIBLE);
                        animationForBlinkingButton();
                        replayButton.setEnabled(false);
                        replayButton.setBackgroundColor(Color.GRAY);
                    }
                })
                .show();
    }
Animation anim;
    public void animationForBlinkingButton(){
            ...
            imageButtonNextLevel.startAnimation(anim);
    }
//Prebacivanje na sljedeci level ----------------------------------------
    TextView levelTextView;
    String levelText = "Level";
    public void goToNextLevelMethod(){
        replayButton.setEnabled(true);
        replayButton.setBackgroundColor(Color.parseColor("#10E6BC"));
        onlyOnceAndLevelDone = true;
        replayMethodAndLevelUp(); // restartovanje polja
        levelTextView.setText(levelText + " " + (int)(currentLevelInGame + 1));
        levelBaza = (int) (currentLevelInGame + 1);
        databaseClass.upisUbazu(levelBaza);  //Upis u bazu kada dodje do novog levela
        imageButtonNextLevel.setVisibility(View.GONE);
        currentLevelInGame++;
    }
//--------------------------------------------------------------------------------
    ...
    public void buttonPoints(View view) {
        AlertDialog.Builder builder = new AlertDialog.Builder(MainActivity.this);

        if(sumOfTotalPoints > 0) {
            builder.setMessage("You have " + sumOfTotalPoints + " points. Well done!").setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialogInterface, int i) {
                    //
                }
            ...
            builder.setMessage("You have " + sumOfTotalPoints + " points.").setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialogInterface, int i) {
                    //
                }
    ...
    public void checkReplay(View view) {
        AlertDialog dialog = new AlertDialog.Builder(this)
                .setTitle("Replay")
                .setMessage("Are you sure you want to restart your level?")
                .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        controlTimerReset = true;
                        replayMethodAndLevelUp();
                    }
                })
    ...
    public void buttonLogout(View view) {
        AlertDialog dialog = new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        FirebaseAuth.getInstance().signOut(); // odjava sa naloga
                        startActivity(new Intent(getApplicationContext(), LoginActivity.class)); // vracanje na login
                        finish(); //??
                    }
                })
                .setNegativeButton("No", new DialogInterface.OnClickListener() {
        ...
        if (countDownTimer != null && controlTimerReset) {
            ...
            countDownTimer = new CountDownTimer(duration, 1000) {
                ...
                public void onFinish() {
                    //when timer goes off
                    if (!onlyOnceAndLevelDone) {
                        lostTextView.setText("LEVEL COMPLITED");
                        lostTextView.setTextColor(Color.YELLOW);
                        lostTextView.setVisibility(View.VISIBLE);
                    } else {
                        lostTextView.setText("GAME OVER");
                        lostTextView.setTextColor(Color.RED);
                        lostTextView.setVisibility(View.VISIBLE);
                    }
                    timerTextView.setTextColor(Color.RED);
                    provjeraDaLiJeTajmerProsao = true;
                }
            }.start();
    }
//---------------------------------------------------------
//Dugme za prelazak na next level
    public void buttonArrow(View view) {
        controlTimerReset = true;
        anim.cancel(); // prekida animaciju slicice za nextlevel, jer ako se ne prekine ostaje prikazana i kad ne treba.
        goToNextLevelMethod();
    }
//-----------------KADA KORISNIK KLIKNE NA BACK DUGME
    ...
    public void onBackPressed() {
        AlertDialog dialog = new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        startActivity(new Intent(MainActivity.this, VerifyUser.class)); // Vraca na mainmenu
                    }
                })
                .setNegativeButton("No", new DialogInterface.OnClickListener() {
                    @Override
    ...
    //------------------------------------------------------------------------------
    //metoda za replay i novi level
    public void replayMethodAndLevelUp(){
        //Namjesta timer za level---------------
        provjeraDaLiJeTajmerProsao = false;
        lostTextView.setVisibility(View.GONE);
        vrijemeZaLevelUMinutama = 1;
        timerTextView.setTextColor(Color.WHITE);
        timerMethod();
        //------------------------------------------
        Arrays.fill(arrayOfUsedLetters, 102);
        Arrays.fill(words, "");  // ispraznjava niz sa slovima
        counterForShades = 0;
        Random rand = new Random();
        for (int k = 0; k < arrayOfNumbers.length; k++) {
            arrayOfNumbers[k] = rand.nextInt(33);
            arrayOfCurrentLetters[k] = String.valueOf(arrayLetters[arrayOfNumbers[k]]);
        }

        for (int j = 0; j < 100;) {
            for (int k = 0; k < 10; k++) {
                for (int u = 0; u < 10; u++,j++) {
                    matrixOfCurrentLetters[k][u] = arrayOfCurrentLetters[j];
                }
            }
        }
        counterForWords = 0; brojac = 0; sumOfTotalPoints = 0;
        gridAdapter = new GridAdapter(MainActivity.this, arrayOfCurrentLetters);
        gridView.setAdapter(gridAdapter);
    }
